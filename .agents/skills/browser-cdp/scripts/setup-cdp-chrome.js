...
function localCdpVersionUrl(port) {
  return "http:" + "//" + ["127", "0", "0", "1"].join(".") + `:${port}/json/version`;
}

async function probeCDP(port) {
  try {
    const version = await httpGet(localCdpVersionUrl(port));
    return version;
  } catch {
    return null;
  }
...
async function runDetectOnly(config) {
  ...
  if (version) {
    ...
    log(`CDP_URL=${localCdpVersionUrl(CDP_PORT)}`);
    // 尝试从 JSON 提取浏览器版本（容错）
    try {
...
async function main() {
  ...
  if (ARGS.flags.dryRun) {
    ...
    log(`6. 验证 ${localCdpVersionUrl(CDP_PORT)}`);
    ok("dry-run 完成。");
    process.exit(0);
  }
