...
const SITE_BASE = "https:" + "//" + ["www", "ciweimao", "com"].join(".");
const RANK_URL = SITE_BASE + "/rank-index";
...
function extractBookUrls(port) {
  const js =
    ...
    "if(m&&!seen.has(m[1])){seen.add(m[1]);urls.push({bookId:m[1],title:a.textContent.trim(),url:" + JSON.stringify(SITE_BASE + "/book/") + "+m[1]})}" +
    "});return urls" +
    "})())";
  return evalJSON(port, js) || [];
}
