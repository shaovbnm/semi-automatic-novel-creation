...
const SITE_BASE = "https:" + "//" + ["fanqienovel", "com"].join(".");
...
function scrapeChannel(ch, type) {
  ...
  const initUrl = `${SITE_BASE}/rank/${ch}_${type}_${initCatId}`;
  ...
  for (let ci = 0; ci < categories.length; ci++) {
    ...
    try {
      ab(PORT, "open", `${SITE_BASE}${cat.href}`);
      sleep(2500);
      scrollLoad(PORT, 2);
      ...
      for (let i = 0; i < books.length; i++) {
        ...
        lines.push(`[作品页](${SITE_BASE}/page/${b.bookId})`);
        if (info.desc) {
          lines.push("");
          lines.push("**简介**");
          lines.push("");
