...
const SITE_BASE = "https:" + "//" + ["www", "qimao", "com"].join(".");
const RANK_URL = SITE_BASE + "/paihang";
...
function extractBookUrls(port) {
  const js =
    ...
    "if(m&&!seen.has(m[1])){seen.add(m[1]);urls.push({bookId:m[1],title:a.textContent.trim(),url:" + JSON.stringify(SITE_BASE + "/shuku/") + "+m[1]+'/'})}" +
    "});return urls" +
    "})())";
  return evalJSON(port, js) || [];
}
